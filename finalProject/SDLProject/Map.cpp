...
bool Map::is_solid(glm::vec3 position, float *penetration_x, float *penetration_y)
{
    ...
    *penetration_y = 0;
    ...
    // Out of map bounds -> treat as non-solid
    ...
    int tile_x = floor((position.x + (m_tile_size / 2.0f)) / m_tile_size);
    int tile_y = -(ceil(position.y - (m_tile_size / 2.0f))) / m_tile_size; // y-flipped

    if (tile_x < 0 || tile_x >= m_width || tile_y < 0 || tile_y >= m_height) return false;
    
    // Look up which tile we're standing on
    int tile = m_level_data[tile_y * m_width + tile_x];

    // Define which tiles are solid
    bool is_solid_tile = (tile == 2 || tile == 15 || tile == 30 || tile == 41|| tile == 20|| tile == 55|| tile == 50|| tile == 46|| tile == 66|| tile == 67);
    if (!is_solid_tile) return false;
    ...
    *penetration_x = (m_tile_size / 2.0f) - fabs(position.x - tile_center_x);
    *penetration_y = (m_tile_size / 2.0f) - fabs(position.y - tile_center_y);
