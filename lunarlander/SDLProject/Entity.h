...
enum EntityType { PLATFORM, PLAYER, ENEMY, LAVA  };
...
public:
    ...
    //Set x acceleration
    void const set_acceleration_x(float new_acceleration) { m_acceleration[0] = new_acceleration; }
    
    //Change going left and right status
    void const toggle_left(bool status){going_left = status;}
    void const toggle_right(bool status){going_right = status;}
    ...
    //WIN AND LOSE CHECKERS
    void const won_game(){won = true;}
    void const lost_game(){lost = true;}
    bool const win_status(){return won;}
    bool const loss_status(){return lost;}
